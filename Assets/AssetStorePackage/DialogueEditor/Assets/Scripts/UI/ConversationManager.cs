...
namespace DialogueEditor
{
    public class ConversationManager : MonoBehaviour
    {
        ...
        // Language support
        private string m_currentCharacterName_CN = "";
        private string m_currentCharacterName_EN = "";
        ...
        {
            // 当语言改变时，实时更新对话文本
            UpdateDialogueTextForCurrentLanguage();
        }
        
        private void UpdateDialogueTextForCurrentLanguage()
        {
            // 如果有当前对话，根据语言设置更新文本
            if (m_currentSpeech != null)
            {
                ...
                if (Global.CurrentLanguage.Value == GlobalEnums.Language.English && !string.IsNullOrEmpty(m_currentSpeech.Text_En))
                {
                    textToDisplay = m_currentSpeech.Text_En;
                }
                else
                {
                    textToDisplay = m_currentSpeech.Text;
                }
                
                if (Global.CurrentLanguage.Value == GlobalEnums.Language.English && !string.IsNullOrEmpty(m_currentSpeech.Name_En))
                {
                    nameToDisplay = m_currentSpeech.Name_En;
                }
                else
                {
                    nameToDisplay = m_currentSpeech.Name;
                }
                ...
                if (!string.IsNullOrEmpty(textToDisplay))
                {
                    ...
                    if (ScrollText)
                    {
                        m_targetScrollTextCount = textToDisplay.Length + 1;
        ...
        {
            Debug.Log($"ConversationManager: 开始对话 - {conversation.name}");
        ...
        private void SetupSpeech(SpeechNode speech)
        {
            ...
            string nameToDisplay = "";
            if (Global.CurrentLanguage.Value == GlobalEnums.Language.English && !string.IsNullOrEmpty(speech.Name_En))
            {
                nameToDisplay = speech.Name_En;
            }
            else
            {
                nameToDisplay = speech.Name;
            }

            // Set text based on current language
            string textToDisplay = "";
            if (Global.CurrentLanguage.Value == GlobalEnums.Language.English && !string.IsNullOrEmpty(speech.Text_En))
            {
                textToDisplay = speech.Text_En;
            }
            else
            {
                textToDisplay = speech.Text;
            }
            
            // 保存当前的中英文文本，用于语言切换
            m_currentChineseText = speech.Text;
            m_currentEnglishText = speech.Text_En;
            
            m_currentCharacterName_CN = speech.Name;
            m_currentCharacterName_EN = speech.Name_En;
